...
func _testMysqlInitSqlConnect(t *testing.T, testName string) *prom.SqlConnect {
...
func TestInitMysqlTable(t *testing.T) {
	testName := "TestInitMysqlTable"
	teardownTest := setupTest(t, testName, setupTestMysql, teardownTestMysql)
	...
	for i := 0; i < 2; i++ {
		if err := InitMysqlTable(testSqlc, testTable, colDef); err != nil {
...
var setupTestMysql = func(t *testing.T, testName string) {
	testSqlc = _testMysqlInitSqlConnect(t, testName)
	testSqlc.GetDB().Exec(fmt.Sprintf("DROP TABLE IF EXISTS %s", testTable))
	colDef := map[string]string{"col_email": "VARCHAR(64)", "col_age": "INT"}
	if err := InitMysqlTable(testSqlc, testTable, colDef); err != nil {
		t.Fatalf("%s failed: %s", testName, err)
...
var teardownTestMysql = func(t *testing.T, testName string) {
	if testSqlc != nil {
